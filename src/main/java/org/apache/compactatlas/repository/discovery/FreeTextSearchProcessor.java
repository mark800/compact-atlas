...
import org.apache.compactatlas.graphdb.api.AtlasIndexQueryParameter;
import org.apache.compactatlas.graphdb.api.AtlasVertex;
import org.apache.compactatlas.graphdb.api.GraphIndexQueryParameters;
...
public class FreeTextSearchProcessor extends SearchProcessor {
    ...
    private GraphIndexQueryParameters prepareGraphIndexQueryParameters(SearchContext context, StringBuilder queryString) {
        ...
        if (isLuceneIndexBackend()) {
            Set<String> keys = context.getIndexedKeys();
            String queryS = queryString.toString();
            String separator = " ";
            StringBuilder sb = new StringBuilder();
            for (String key : keys) {
                if(!key.startsWith(Constants.INTERNAL_PROPERTY_KEY_PREFIX)) {
                    sb.append(key).append(":");
                    sb.append(queryS);
                    sb.append(separator);
                }
            }
            String ends = sb.toString().trim();
            return new GraphIndexQueryParameters(Constants.VERTEX_INDEX, ends, 0, parameters);
        }
    ...
    private static boolean isLuceneIndexBackend() {
        try {
            String indexBackEnd = ApplicationProperties.get().getString(ApplicationProperties.INDEX_BACKEND_CONF);

            return ApplicationProperties.INDEX_BACKEND_LUCENE.equalsIgnoreCase(indexBackEnd);
        } catch (AtlasException e) {
            LOG.error("Failed to get application property {}. Assuming Solr index backend", ApplicationProperties.INDEX_BACKEND_SOLR, e);
        }
        return true;
