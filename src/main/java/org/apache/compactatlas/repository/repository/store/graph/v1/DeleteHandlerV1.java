 ...
 */
package org.apache.compactatlas.repository.repository.store.graph.v1;
...
import org.apache.compactatlas.repository.repository.graph.AtlasEdgeLabel;
...
import org.apache.compactatlas.repository.repository.store.graph.v2.EntityGraphRetriever;
import org.apache.compactatlas.intg.DeleteType;
import org.apache.compactatlas.repository.repository.store.graph.v2.tasks.ClassificationTask;
import org.apache.compactatlas.repository.tasks.TaskManagement;
import org.apache.compactatlas.intg.type.*;
...
import static org.apache.compactatlas.intg.model.TypeCategory.*;
...
import static org.apache.compactatlas.intg.model.instance.AtlasEntity.Status.PURGED;
...
import static org.apache.compactatlas.repository.repository.store.graph.v2.AtlasGraphUtilsV2.*;
...
public abstract class DeleteHandlerV1 {
    ...
    public void deleteRelationships(Collection<AtlasEdge> edges, final boolean forceDelete) throws AtlasBaseException {
        ...
        for (AtlasEdge edge : edges) {
            ...
            boolean needToSkip = !isInternal && (AtlasGraphUtilsV2.getState(edge) == (isPurgeRequested ? ACTIVE : DELETED));

            if (needToSkip) {
                if (LOG.isDebugEnabled()) {
                    if(isPurgeRequested) {
                        LOG.debug("Skipping purging of edge={} as it is active or already purged", AtlasGraphUtilsV2.getIdFromEdge(edge));
                    } else{
                        LOG.debug("Skipping deletion of edge={} as it is already deleted", AtlasGraphUtilsV2.getIdFromEdge(edge));
                    }
                }
            ...
            for (AtlasStructType.AtlasAttribute attributeInfo : entityType.getOwnedRefAttributes()) {
                ...
                if (typeCategory == OBJECT_ID_TYPE) {
                    if (attributeInfo.getAttributeDef().isSoftReferenced()) {
                        ...
                        if (edge == null || (AtlasGraphUtilsV2.getState(edge) == (isPurgeRequested ? ACTIVE : DELETED))) {
                            continue;
                        }
    ...
    private void addTagPropagation(AtlasVertex fromVertex, AtlasVertex toVertex, AtlasEdge edge) throws AtlasBaseException {
        ...
        if (taskManagement != null && DEFERRED_ACTION_ENABLED) {
            for (AtlasVertex classificationVertex : classificationVertices) {
                createAndQueueTask(ClassificationPropagateTaskFactory.CLASSIFICATION_PROPAGATION_ADD, toVertex, classificationVertex.getIdForDisplay(), relationshipGuid);
            }
        } else {
            ...
            if (CollectionUtils.isNotEmpty(propagatedEntityVertices)) {
                if (LOG.isDebugEnabled()) {
                    LOG.debug("Propagate {} tags: from {} entity to {} entities", classificationVertices.size(), AtlasGraphUtilsV2.getTypeName(fromVertex), propagatedEntityVertices.size());
                }
    ...
    public List<AtlasVertex> addTagPropagation(AtlasVertex classificationVertex, List<AtlasVertex> propagatedEntityVertices) throws AtlasBaseException {
        ...
        if (CollectionUtils.isNotEmpty(propagatedEntityVertices) && classificationVertex != null) {
            ...
            for (AtlasVertex propagatedEntityVertex : propagatedEntityVertices) {
                if (getClassificationEdge(propagatedEntityVertex, classificationVertex) != null) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(" --> Classification edge already exists from [{}] --> [{}][{}] using edge label: [{}]",
                                  AtlasGraphUtilsV2.getTypeName(propagatedEntityVertex), AtlasGraphUtilsV2.getTypeName(classificationVertex), AtlasGraphUtilsV2.getTypeName(associatedEntityVertex), classificationName);
                    }
                    ...
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(" --> Propagated classification edge already exists from [{}] --> [{}][{}] using edge label: [{}]",
                                  AtlasGraphUtilsV2.getTypeName(propagatedEntityVertex), AtlasGraphUtilsV2.getTypeName(classificationVertex), AtlasGraphUtilsV2.getTypeName(associatedEntityVertex), CLASSIFICATION_LABEL);
                    }

                    continue;
                }

                String          entityTypeName = AtlasGraphUtilsV2.getTypeName(propagatedEntityVertex);
                ...
                if (!classificationType.canApplyToEntityType(entityType)) {
                    if (LOG.isDebugEnabled()) {
                        LOG.debug(" --> Not creating propagated classification edge from [{}] --> [{}][{}], classification is not applicable for entity type",
                                   AtlasGraphUtilsV2.getTypeName(propagatedEntityVertex), AtlasGraphUtilsV2.getTypeName(classificationVertex), AtlasGraphUtilsV2.getTypeName(associatedEntityVertex));
                    }
                ...
                if (LOG.isDebugEnabled()) {
                    LOG.debug(" --> Adding propagated classification: [{}] to {} ({}) using edge label: [{}]", classificationName, AtlasGraphUtilsV2.getTypeName(propagatedEntityVertex),
    ...
    public boolean isRelationshipEdge(AtlasEdge edge) {
        ...
        if (edge != null) {
            String outVertexType = AtlasGraphUtilsV2.getTypeName(edge.getOutVertex());
            String inVertexType  = AtlasGraphUtilsV2.getTypeName(edge.getInVertex());
    ...
    public void deletePropagatedClassification(AtlasVertex entityVertex, String classificationName, String associatedEntityGuid) throws AtlasBaseException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Removing propagated classification: [{} - associatedEntityGuid: {}] from: [{}][{}] with edge label: [{}]",
                       classificationName, associatedEntityGuid, AtlasGraphUtilsV2.getTypeName(entityVertex), getGuid(entityVertex), CLASSIFICATION_LABEL);
        }
    ...
    public void deletePropagatedEdge(AtlasEdge edge) throws AtlasBaseException {
        ...
        if (LOG.isDebugEnabled()) {
            LOG.debug("Removing propagated classification: [{}] from: [{}][{}] with edge label: [{}]", classificationName,
                       AtlasGraphUtilsV2.getTypeName(entityVertex), GraphHelper.getGuid(entityVertex), CLASSIFICATION_LABEL);
        }
            ...
            for (AtlasStructType.AtlasAttribute attributeInfo : structType.getAllAttributes().values()) {
                ...
                switch (attrType.getTypeCategory()) {
                    ...
                    case MAP:
                        ...
                        if (AtlasGraphUtilsV2.isReference(valueTypeCategory)) {
    ...
    private void deleteAllClassifications(AtlasVertex instanceVertex) throws AtlasBaseException {
        ...
        for (AtlasEdge edge : classificationEdges) {
            ...
            if (isClassificationEdge && removePropagations) {
                if (taskManagement != null && DEFERRED_ACTION_ENABLED) {
                    createAndQueueTask(ClassificationPropagateTaskFactory.CLASSIFICATION_PROPAGATION_DELETE, instanceVertex, classificationVertex.getIdForDisplay(), null);
                } else {
